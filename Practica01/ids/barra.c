...
void writeTable(int pos, struct dogType *pet){//funcion para actualizar la tabla hash
    long int wr=0;//long int para el tamaño de bytes del archivo
    int tam, ingre;//tam: estructuras encontradas, ingre: estructuras ingresadas
    FILE *files2=fopen("dataDogs.dat","rb");//abre el archivo para lectura
    if(files2==NULL){//validacion
        printf("Error abriendo archivo dataDogs.dat.\n");
    }else{//puede abrir el archivo sin problema
		fseek(files2, 0L,SEEK_END);//posiciona el puntero al final
		wr=ftell(files2);//asigna el tamaño del archivo
		fseek(files2,0L,SEEK_SET);//posiciona el puntero al comienzo
		fread(&ingre,sizeof(int),1,files2);//lee el entero ingresado 
		int tam = (int)(((wr-sizeof(int))/sizeof(struct dogType)));//calcula el tamaño de estructuras encontradas
		printf("estructuras ingresadas%i\n",ingre);
		printf("estructuras encontradas%i\n",tam);
		printf("tam leido %li\n",wr);
	}
    FILE *files=fopen("dataTemp.dat","wb+");//abre el archivo dataTemp en forma lectura escritura binaria
    if(files==NULL){//valida
        printf("Error abriendo archivo dataTemp.dat.\n");
        return;
    }else{//crea el archivo es escritura y lectura sin problema
        struct dogType *temp;//estructura temporal para leer el archivo
        tam = (int)(((wr-sizeof(int))/sizeof(struct dogType)));//vuelve a asignar el tam de estructuras encontradas
        printf("tam leido %li\n",wr);
        ...
        if(wr <= 0){///va a ingresar un registro por primera vez
            ...
            fseek(files,0L,SEEK_SET);//pone el puntero al comienzo
            int f=1;//numero de f estructuras ingreadas
            fwrite(&f,sizeof(int),1,files);//asigna a f y corre el puntero
            temp = malloc(sizeof(struct dogType));//reserva espacio la estructura 
            for(int rec = 1; rec < pos; rec++){//for para estructuras null
                temp->id = rec;
                temp->next = pos;
                fwrite(temp,sizeof(struct dogType),1,files);
            }//esta en la posicion pos
            fwrite(pet,sizeof(struct dogType),1,files);//ingresa la mascota en pos
            fclose(files);//cierra archivo de lectura y escritura
            rename("dataTemp.dat","dataDogs.dat");//lo renombra
            //printf("OK\n");Debug
            free(temp);//libera el espacio de la estructura
        }else{
